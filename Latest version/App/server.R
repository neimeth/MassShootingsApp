    ...
    observe({
      ...
      if(input$shot == "Wounded"){
        leafletProxy("map", data=datapoints()) %>%
          clearMarkers() %>%
          addCircleMarkers(~lon, ~lat, radius=~Wounded*2, color = 'red', weight = 1,
                     popup = paste0('<a href=', datapoints()$Article, '>', 'Link to News Coverage', '</a>',
                                    '<p>', 'Location = ', datapoints()$Location, '</p>',
                                    '<p>', 'Date = ', datapoints()$Date, '</p>',
                                    '<p>', 'Wounded = ', datapoints()$Wounded, '</p>', 
                                    '<p>', 'Killed = ', datapoints()$Killed, '</p>',
                                    '<a href=http://www.guncontrolforums.com/discussion/316/gun-control-should-we-ban-them>', 'Click HERE to join the discussion', '</a>'
                                    ))
      }
      if(input$shot == "Killed"){
        leafletProxy("map", data=datapoints()) %>%
          clearMarkers() %>%
          addCircleMarkers(~lon, ~lat, radius=~Killed*2, color = 'red', weight = 1,
                           popup = paste0('<a href=', datapoints()$Article, '>', 'Link to News Coverage', '</a>',
                                          '<p>', 'Location = ', datapoints()$Location, '</p>',
                                          '<p>', 'Date = ', datapoints()$Date, '</p>',
                                          '<p>', 'Wounded = ', datapoints()$Wounded, '</p>', 
                                          '<p>', 'Killed = ', datapoints()$Killed, '</p>',
                                          '<a href=http://www.guncontrolforums.com/discussion/316/gun-control-should-we-ban-them>', 'Join a discussion', '</a>'
                           ))
      }
    }) ### End of observe
